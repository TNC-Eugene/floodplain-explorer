...
function ( 	ArcGISDynamicMapServiceLayer, Extent, SpatialReference, Query, QueryTask, declare, FeatureLayer, 
        ...
        return declare(null, {
			esriApiFunctions: function(t){
				...
				q.outFields = ['OBJECTID_1','IslandName'];
				q.where = "OBJECTID_1 > -1"
				t.islandPolygons.selectFeatures(q,FeatureLayer.SELECTION_NEW);
				
				// var qL = new Query();
				// qL.returnGeometry = false;
				// qL.outFields = ['xmin','xmax'];
				// qL.where = "xmax > -1"
				// t.islandChangeLong.selectFeatures(qL, FeatureLayer.SELECTION_NEW);
				...
				// t.map.addLayer(t.islandChangeLong);
				// t.map.addLayer(t.islandChangeShort);
				...
				t.islandPolygons_click.on('selection-complete',lang.hitch(t,function(evt){
					if(evt.features.length > 0){
						if(t.obj.trigger == 'mapClick'){
							...
							var p = 'x'
							$('#' + t.id + 'ch-ISL').val(islandValue).trigger('chosen:updated').trigger('change',p);
						}
